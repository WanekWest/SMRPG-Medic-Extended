...
ConVar g_hCVIncrease, g_hCVInterval, g_hCVRadius, g_hCVRadiusIncrease, hLevelForBonus, hExpAmount;

bool g_bIsCstrike;

Handle g_hMedicTimer;

int g_hLevelForBonus, g_hExpAmount;
...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		SMRPG_RegisterUpgradeType("Medic", UPGRADE_SHORTNAME, "Heals team mates around you.", 0, true, 15, 1500, 1100);
		...
		hLevelForBonus = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_medic_req_level_bonus", "100", "To which level, it will work without.", _, true, 1.0);
		hLevelForBonus.AddChangeHook(OnMedicBonReqLvlCh);
		g_hLevelForBonus = hLevelForBonus.IntValue;

		hExpAmount = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_medic_add_exp_amount", "50", "Exp which will give by bonus(Value*Level).", _, true, 1.0);
		hExpAmount.AddChangeHook(OnMedicAddExpBonCh);
		g_hExpAmount = hExpAmount.IntValue;
	}
}
...
public Action Timer_ApplyMedic(Handle timer, any data)
{
	...
	for(int i=1;i<=MaxClients;i++)
	{
		...
		for(int m=1;m<=MaxClients;m++)
		{
			...
			if(SMRPG_GetClientLevel(i) < g_hLevelForBonus)
			{
				int exp = g_hExpAmount * SMRPG_GetClientLevel(i);
				SMRPG_AddClientExperience(i, exp, "Medic", false, -1);
				PrintToChat(i, "Вы получили %i опыта за лечение союзника!", exp);
			}
...
public void OnMedicBonReqLvlCh(ConVar hCvar, const char[] szOldValue, const char[] szNewValue)
{
	g_hLevelForBonus = hCvar.IntValue;
}

public void OnMedicAddExpBonCh(ConVar hCvar, const char[] szOldValue, const char[] szNewValue)
{
	g_hExpAmount = hCvar.IntValue;
}